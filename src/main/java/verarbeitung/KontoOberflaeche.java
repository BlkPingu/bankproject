...
public class KontoOberflaeche extends BorderPane {
    ...
    public KontoOberflaeche(KontoController kontoController)
    {
        ...
        nummer.setText(kontoController.konto.getKontonummerFormatiert());
        ...
        stand.textProperty().bind(kontoController.konto.getKontostandReadOnlyProperty().asString());
        kontoController.konto.isKontoImMinus.addListener((observable, oldValue, newValue) -> {
            if(newValue){
        ...
        gesperrt.selectedProperty().bindBidirectional(kontoController.konto.gesperrtProperty);
        ...
        adresse.textProperty().bindBidirectional(kontoController.konto.getInhaber().getAdressStringProperty());
        ...
        betrag = new Spinner<>(50.0, 500.0, 50.0, 50.0);
        aktionen.getChildren().add(betrag);
        einzahlen = new Button("Einzahlen");
        //einzahlen.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> kontoController.einzahlen(betrag.getValue()));
        aktionen.getChildren().add(einzahlen);
        abheben = new Button("Abheben");
        //abheben.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> kontoController.abheben(betrag.getValue()));
